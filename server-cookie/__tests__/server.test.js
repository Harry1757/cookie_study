...
describe('Authentication - Server', () => {
  ...
  describe('Authentication - Server', () => {
    ...
    describe('⛳️ POST /users/login', () => {
      ...
      it('id cookie value should be set as userInfo.id after login successfully', async () => {
        const response = await agent.post('/users/login').send({
          userId: 'kimcoding',
          password: '1234',
        });
        ...
        expect(resCookies[0]).include('id=1');
      });
      ...
      it('cookies should have Secure and SameSite option', async () => {
        ...
        expect(resCookies[0]).include('HttpOnly');
        expect(resCookies[0]).include('Secure');
        expect(resCookies[0]).include('SameSite=None');
      });
    });
    ...
    describe('⛳️ POST /users/logout', () => {
      ...
      it('id cookie value should be cleared after logout', async () => {
        let loginResponse = await agent.post('/users/login').send({
          userId: 'kimcoding',
          password: '1234',
        });
        let resCookies = loginResponse.header['set-cookie'];
